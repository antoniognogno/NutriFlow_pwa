...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (mealToRegenerate && Array.isArray(existingMeals)) {
      ...
      let ingredientsPromptSection = customIngredients ? `- Ingredienti da usare: ${customIngredients}.` : '';
      let breakfastPromptSection = breakfastPreference ? `- Preferenza colazione: ${breakfastPreference}.` : '';
      let hintPromptSection = recipeHint ? `- Suggerimento piatto: "${recipeHint}".` : '';
